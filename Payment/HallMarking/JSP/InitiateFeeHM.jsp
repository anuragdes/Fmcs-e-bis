...
function deleteFeeId(feeNumId){
	...
	if (result) {
		 document.forms[0].method="POST";
	 	 document.forms[0].action="/MANAK/DeleteInitateFee?FeeId="+feeNumId+"&eappid="+eappid+"&ebranchid="+ebranchid+"&ecml="+ecml;
		 document.forms[0].submit(); 
		}
}
function getFeeAmount(feeid,count){
	$.ajax({
		url: "/MANAK/getFeeAmountHM?FeeId="+feeid,
		type : "POST",
...
<div class="container">
<form:form cssClass="form-horizontal" method="post" id="frm" name="frm" action="InitiateFeeHMSubmit" commandName="model">
...
<fieldset>
  	    ...
  	    <c:if test="${PreviousDuesDetailHMlist.size()>0}">
             <table class="footable table bg-head table-bordered table-striped" data-page-size="5">
                ...
                <td>
                <button type="button" name="deleteFee" id="deleteFee${loop.index}" class="btn btn-danger" onclick="deleteFeeId('${listApp.num_Id}')" ><i class="glyphicon glyphicon-remove"></i></button>
                </td>
                </tr>
                </c:forEach>
               </tbody>
