...
<table id="tblresult" class="blue bg-head table table-striped table-bordered table-hover dt-responsive nowrap dataTable no-footer dtr-inline collapsed">
	<thead class="filters">
		<tr style="background-color: #338ec9; color: #ffffff;">
			...
			<%--<th><spring:message code="label.branchID"></spring:message></th>--%>
			<%--<th><spring:message code="label.appcml"></spring:message></th>--%>
				<%--<th>CML No.</th>--%>
			<%--<th><spring:message code="label.cml"></spring:message></th>--%>
			...
			<%--<th><spring:message code="label.paidID"></spring:message></th>--%>
				<%--<th>Amount (INR)</th>--%>
			...
			<th><spring:message code="label.paymentDate"></spring:message></th>
			<th><spring:message code="label.trxnNo"></spring:message></th>
			<th><spring:message code="label.receipt_no"></spring:message></th>
	...
	<tbody id="tblresultbody" class="container">
		<c:if test="${applicationList != null }">
			...
			<c:forEach items="${applicationList}" var="listApp">
				<tr id="${listApp.num_FeeDetail_Id11}_tr">
					<td><c:set var="sno" value="${sno + 1}" />${sno}</td>
					<%--<td><c:out value="${listApp.num_branch_id}"></c:out></td>--%>
					...
					<td><span><fmt:formatDate pattern="yyyy-MM-dd" value="${listApp.dt_Paymentdate }" /></span></td>
					<td>${listApp.transaction_number }</td>
					<%--<td>${listApp.num_receipt_no }</td>--%>
					<td><a target="_blank" href="/MANAK/viewPreviousFeeReceipt?docName=FeeReceipt_${listApp.transaction_number }.pdf" target="_blank"><span>${listApp.num_receipt_no }</span></a></td>
					<%--<td class="links"></td>--%>
				</tr>
			</c:forEach>
		</c:if>
